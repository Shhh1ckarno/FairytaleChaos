...
public class BattleManager : MonoBehaviour
{
    ...
    public void StartGame()
    {
        currentRound = 0;
        maxFearPoints = 0;
        currentFearPoints = 0; // Добавим обнуление FP

        // 1. Сброс здоровья
        if (HealthManager.Instance != null) HealthManager.Instance.InitializeHealth();

        // 2. ИСПРАВЛЕНИЕ: ВЫЗЫВАЕМ НАЧАЛЬНЫЙ ДОБОР КАРТ ТОЛЬКО ЗДЕСЬ!
        if (HandManager.Instance != null)
        {
            // Предполагается, что HandManager.DrawStartingHand() добирает 4 карты.
    ...
    public void StartTurn(PlayerType player)
    {
        ...
        if (player == PlayerType.Player)
        {
            ...
            // ИСПРАВЛЕНИЕ: ЕЖЕХОДНЫЙ ДОБОР КАРТЫ ПРОИСХОДИТ ТОЛЬКО СО 2-ГО РАУНДА
