...
public class CardController : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    {
        hasBlock = true;
        if (cardDisplay != null) cardDisplay.UpdateStatusText("«¿ ¿À ¿", true); // TEMPERING
    ...
    {
        hasRebirth = true;
        if (cardDisplay != null) cardDisplay.UpdateStatusText("œ≈–≈œÀ¿¬ ¿", true); // REBIRTH
    ...
    public void ApplyOnFire(int bonusAtk)
    {
        ...
        if (cardDisplay != null)
        {
            cardDisplay.UpdateStatusText("œŒƒ∆Œ√", true); // IGNITION
            cardDisplay.UpdateAttackText(GetTotalAttack());
        }
